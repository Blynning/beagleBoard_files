...
int get_Temp(char* tempAddr){
        ...
        // få temperatur værdig fra reg32 (reg16poi[0] tager de første 16bits)
        int temperature_raw = bswap_16(reg16poi[0]); // biteswap for at ændre fra big endien til little endien

        // den reelle temperatur i bits -> til double så man får det i decimal
        sprintf(tempAddr, "%0.1fC",(temperature_raw & BIT_MASK_HEX) / 16.0);
        return(0);
}
